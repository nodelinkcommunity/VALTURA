...
const ACCESS_CONTROL_ABI = [
  ...
  'function setHiddenPosition(address user, uint256 posId, bool hidden)',
...
const REDEMPTION_MANAGER_ABI = [
  ...
  'function getOrder(uint256 orderId) view returns (address user, uint256 posId, uint256 amount, uint256 createdAt, uint8 status, uint8 rejectReason)',
...
async function getRedemptionOrder(orderId) {
  const rm = getRedemptionManager();
  const result = await rm.getOrder(orderId);
  return {
    orderId: Number(orderId),
    user: result[0],
    posId: Number(result[1]),
    amount: fromUSDT(result[2]),
    createdAt: Number(result[3]),
    status: Number(result[4]),
    rejectReason: Number(result[5]),
  };
}

async function findPendingRedemptionOrder(userAddress, posId, amount) {
  const rm = getRedemptionManager();
  const count = Number(await rm.orderCount());
  const wantedUser = String(userAddress || '').toLowerCase();
  const wantedPos = Number(posId);
  const wantedAmount = toUSDT(amount);

  for (let i = count - 1; i >= 0; i--) {
    const result = await rm.getOrder(i);
    if (
      String(result[0] || '').toLowerCase() === wantedUser &&
      Number(result[1]) === wantedPos &&
      result[2] === wantedAmount &&
      Number(result[4]) === 0
    ) {
      return {
        orderId: i,
...
async function verifyDepositTransaction(txHash, expectedAmount, userWallet) {
  try {
    ...
    const depositedEvents = receipt.logs
      .filter((log) => log.address && log.address.toLowerCase() === vaultAddr)
      .map((log) => {
...
async function verifyLeaderGrantTransaction(txHash, adminWallet, userWallet, expectedAmount, expectedHidden, fallbackPreGrantCount = null) {
  const provider = getProvider();
  const [receipt, tx] = await Promise.all([
    provider.getTransactionReceipt(txHash),
    provider.getTransaction(txHash),
  ...
  if (!receipt || receipt.status !== 1) {
    throw new Error('Leader grant transaction failed or not found');
  }

  const vault = getVault();
  const vaultAddress = config.polygon.contracts.valturVault.toLowerCase();
  const normalizedAdmin = adminWallet.toLowerCase();
  const normalizedUser = userWallet.toLowerCase();

  if (!tx || !tx.to || tx.to.toLowerCase() !== vaultAddress) {
    throw new Error('Leader grant transaction target mismatch');
  }
  if (!tx.from || tx.from.toLowerCase() !== normalizedAdmin) {
    throw new Error('Leader grant signer mismatch');
  }

  const parsedTx = vault.interface.parseTransaction({ data: tx.data, value: tx.value });
  if (!parsedTx || parsedTx.name !== 'grantLeaderPackage') {
    throw new Error('Expected grantLeaderPackage transaction');
  }

  if (parsedTx.args[0].toLowerCase() !== normalizedUser) {
    throw new Error('Leader grant user mismatch');
  }

  const txAmount = fromUSDT(parsedTx.args[1]);
  if (Math.abs(txAmount - expectedAmount) > 0.01) {
    throw new Error('Leader grant amount mismatch');
  }

  if (Boolean(parsedTx.args[2]) !== Boolean(expectedHidden)) {
    throw new Error('Leader grant hidden flag mismatch');
  }

  const grantEvent = receipt.logs
    .filter((log) => log.address && log.address.toLowerCase() === vaultAddress)
    .map((log) => {
      try { return vault.interface.parseLog(log); } catch (e) { return null; }
    })
    .find((event) => event && event.name === 'LeaderGranted');

  if (!grantEvent) {
    throw new Error('No LeaderGranted event found');
  }
  if (grantEvent.args[0].toLowerCase() !== normalizedUser) {
    throw new Error('LeaderGranted event user mismatch');
  }
  if (Math.abs(fromUSDT(grantEvent.args[1]) - expectedAmount) > 0.01) {
    throw new Error('LeaderGranted event amount mismatch');
  }
  if (Boolean(grantEvent.args[2]) !== Boolean(expectedHidden)) {
    throw new Error('LeaderGranted event hidden flag mismatch');
  }

  const currentCount = Number(await vault.getUserPositionCount(userWallet));
  let countBefore = null;
  if (receipt.blockNumber > 0) {
    try {
      countBefore = Number(await vault.getUserPositionCount(userWallet, { blockTag: receipt.blockNumber - 1 }));
    } catch (historyErr) {
      if (Number.isInteger(fallbackPreGrantCount) && fallbackPreGrantCount >= 0) {
        countBefore = fallbackPreGrantCount;
      } else {
        throw new Error('Could not derive leader grant position index');
      }
    }
  }
  if (!Number.isInteger(countBefore) || countBefore < 0) {
    throw new Error('Invalid leader grant position index');
  }
  if (currentCount <= countBefore) {
    throw new Error('Leader grant position count did not increase');
  }

  const positionIndex = countBefore;
  const position = await vault.getPosition(userWallet, positionIndex);
  if (Math.abs(fromUSDT(position.amount) - expectedAmount) > 0.01) {
    throw new Error('Granted position amount mismatch');
  }
  if (Number(position.packageType) !== 6) {
    throw new Error('Granted position package type mismatch');
  }
  if (!position.active || !position.isGranted) {
    throw new Error('Granted position state mismatch');
  }
  ...
  return {
    ...
    positionIndex,
    amount: txAmount,
    adminWallet: normalizedAdmin,
    userWallet: normalizedUser,
    hidden: Boolean(expectedHidden),
  };
}

async function verifyHiddenPositionTransaction(txHash, adminWallet, userWallet, posId, expectedHidden) {
  const provider = getProvider();
  const [receipt, tx] = await Promise.all([
    provider.getTransactionReceipt(txHash),
    provider.getTransaction(txHash),
  ...
  if (!receipt || receipt.status !== 1) {
    throw new Error('Hidden position transaction failed or not found');
  }

  const accessControl = getAccessControl();
  const accessControlAddress = config.polygon.contracts.accessControl.toLowerCase();
  const normalizedAdmin = adminWallet.toLowerCase();
  const normalizedUser = userWallet.toLowerCase();

  if (!tx || !tx.to || tx.to.toLowerCase() !== accessControlAddress) {
    throw new Error('Hidden position transaction target mismatch');
  }
  if (!tx.from || tx.from.toLowerCase() !== normalizedAdmin) {
    throw new Error('Hidden position signer mismatch');
  }

  const parsedTx = accessControl.interface.parseTransaction({ data: tx.data, value: tx.value });
  if (!parsedTx || parsedTx.name !== 'setHiddenPosition') {
    throw new Error('Expected setHiddenPosition transaction');
  }
  if (parsedTx.args[0].toLowerCase() !== normalizedUser) {
    throw new Error('Hidden position user mismatch');
  }
  if (Number(parsedTx.args[1]) !== Number(posId)) {
    throw new Error('Hidden position index mismatch');
  }
  if (Boolean(parsedTx.args[2]) !== Boolean(expectedHidden)) {
    throw new Error('Hidden position flag mismatch');
  }

  const hiddenEvent = receipt.logs
    .filter((log) => log.address && log.address.toLowerCase() === accessControlAddress)
    .map((log) => {
      try { return accessControl.interface.parseLog(log); } catch (e) { return null; }
    })
    .find((event) => event && event.name === 'PositionHiddenSet');

  if (!hiddenEvent) {
    throw new Error('No PositionHiddenSet event found');
  }
  if (hiddenEvent.args[0].toLowerCase() !== normalizedUser) {
    throw new Error('PositionHiddenSet event user mismatch');
  }
  if (Number(hiddenEvent.args[1]) !== Number(posId)) {
    throw new Error('PositionHiddenSet event index mismatch');
  }
  if (Boolean(hiddenEvent.args[2]) !== Boolean(expectedHidden)) {
    throw new Error('PositionHiddenSet event flag mismatch');
  }

  const isHidden = await accessControl.isHidden(userWallet, posId);
  if (Boolean(isHidden) !== Boolean(expectedHidden)) {
    throw new Error('On-chain hidden position state mismatch');
  }

  return {
    txHash,
    positionIndex: Number(posId),
...
module.exports = {
  ...
  verifyLeaderGrantTransaction,
  verifyHiddenPositionTransaction,
