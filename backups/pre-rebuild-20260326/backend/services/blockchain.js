...
async function verifyDepositTransaction(txHash, expectedAmount) {
  try {
    ...
    // Check the transaction went to our vault
    ...
    for (const log of receipt.logs) {
      if (log.address.toLowerCase() === config.polygon.usdtAddress.toLowerCase()) {
        if (log.topics[0] === transferTopic) {
          try {
            const decoded = usdtIface.decodeEventLog("Transfer", log.data, log.topics);
            const to = ethers.getAddress("0x" + log.topics[2].slice(26));
            const from = ethers.getAddress("0x" + log.topics[1].slice(26));
            const value = decoded[0] !== undefined ? decoded[0] : ethers.toBigInt(log.data);
    ...
    if (depositAmount === 0) {
      return { valid: false, reason: "No USDT transfer to vault found in transaction" };
    }

    // Allow small rounding tolerance (0.01 USDT)
    if (Math.abs(depositAmount - expectedAmount) > 0.01) {
      return { valid: false, reason: "Amount mismatch: on-chain=" + depositAmount + " expected=" + expectedAmount };
    }

    return { valid: true, amount: depositAmount, from: fromAddress };
  } catch (err) {
